...
to check if there are any pending jobs, then runs them for maximum their budget time. If there is no
...
\label{s:cyclic-executive}), with parameters computed offline. If no level-A tasks are eligible to
...
asses their suitability for mixed criticality systems.
...
Finally, of the most relevant to mixed-criticality systems, is enforcement: if the reservation being
