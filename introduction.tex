...
criticality tasks where errors only lead to degradation in service, or not at all, this excess
...
is not even a trusted service: it is subject to attacks, like denial-of-service attacks. 
...
Consequently, through shared resources, mixed-criticality are integral to future cyber-physical
systems which require resource sharing, something that is not possible with traditional separating
...
the same concepts from a systems perspective, and presents a survey of existing commercial, open-source
...
We then draw on all the previous chapters to design mechanisms for efficiently supporting mixed
