...
exception if a task holds a resource for too long, although this requires an exact scheduling
details to be provided about the task and resource. ARINC alone provides real temporal isolation for
tasks sharing resources, however like its scheduler, the support is limited to fixed, static
...
safety-criticality systems, temporal isolation and resource sharing mechanisms remain important to
...
\gls{CBS} and other algorithms, support for temporal isolation in 
...
applications is limited to the ARINC653 approach discussed in \cref{s:arinc}. 
...
a higher authority was required to replenish the meter.

L4 kernels~\citep{Elphinstone_Heiser_13} extended this with timeslices, although in different ways.
...
Meters and timeslices provide a unit of time, but do not restrict when that time must be consumed
by, and have insufficient policy freedom in that they are recharged immediately, providing no
mechanism for an upper bound on execution. 
As a result, in a system where time is treated as a first-class resource, 
the timeslice/meter is not an appropriate mechanism alone for building mixed-criticality
systems with temporal isolation guarantees.

\subsection{Reservations}
% TODO finish unifying

Many research \glspl{OS} have provided mechanisms for processor reservations, which thread(s)
execute upon to consume time. The reservation parameters, charging and enforcement policies vary wildly,
and we survey them here. 
...
scheduler such that CBS can be implemented at user-level. 

For the fixed-priority \glspl{OS}, two broad
