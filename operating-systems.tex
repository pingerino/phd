...
% TODO break up into sections: dicuss resource sharing, isolation approaches
...
Scheduling policies specified by \gls{POSIX} are shown in \Cref{tab:posix-sched}. 
...
Resource sharing in the \gls{POSIX} \gls{OS} interface is permitted through mutexes, which can be used to build higher synchronisation protocols.
\Cref{tab:posix-mutex} shows the specified protocols. 
...
Linux version 3.14 saw the introduction of an \gls{EDF} scheduling class to Linux~\citep{Corbet_2009}, which is between the fair and the fixed priority scheduling classes.
...
\citet{Steinburg_WH_2005} scheduling context consisted of a time quanta and a priority and were donated across \gls{IPC}.
