...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{endpoint}
...
on this endpoint as if the kernel had conducted a \code{call()}: the kernel blocks the faulting
...
Notification objects provide the mechanism for semaphores in \selfour. 

The signals are non-blocking, as the badge of the notification capability is simply
...
\begin{table} 
    ...
    \begin{tabular}{p{0.2\textwidth}p{0.8\textwidth}}\toprule
        \emph{System call} & Action \\\midrule
        \texttt{send}   & Send a message, bitwise ORING blocking until it is consumed. \\ 
        \texttt{nbsend} & Send a message, but only if it is consumed immediately (\ie a thread is
        already waiting on this endpoint for a message).  \\
        \texttt{recv}   & Block until a message is available to be consumed from this endpoint.  \\
        \texttt{nbrecv} & Poll for a message---consume a message from this endpoint, but only if it is available
...
\begin{figure}
    ...
    \caption{Notification object state transitions based on invocations.}
    \label{f:endpoint}
\end{figure}
...
% cover this under notification binding
