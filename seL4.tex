...
\code{recv()} by \emph{resume} capabilities. Pioneered in KeyKOS~\citep{Bomberger_FFHLS_92} and
further appearing in EROS~\citep{Shapiro_SF_99}, resume
capabilities are single-use capabilities, generated when a message is sent by \code{call()} and
...
Threads can surrender their current tick using the \code{yield} system call, which simulates
