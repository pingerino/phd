...
highlight deficiencies and motivate our changes.  We will outline the existing scheduler, the API
curiosity that is \yield, and how \gls{IPC} interacts with scheduling, followed by an analysis of
...
Several optimisations exist in the kernel for performance, including lazy and benno
scheduling. 
\emph{Lazy} scheduling is the first optimisation, and is used whenever a thread blocks and wakes
...
directly switch to and the scheduler is avoided all together. Otherwise the receiver is moved from
...
threads except the current thread are in the scheduler. By doing so we avoid manipulating the
...
In order to achieve temporal isolation, \selfour provides a top-level hierarchical scheduler
...
The domain scheduler was introduced in order to complete the proof of confidentiality \TODO{cite
paper}.
...
\TODO{shared resource temporal isolation not possible}.
