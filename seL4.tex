...
Recall \selfour from its introduction in \cref{s:background_sel4}: it is particularly suited to
safety-critical, real-time systems as it has been formally
verified for correctness~\citep{Klein_AEMSKH_14}, integrity~\citep{Sewell_WGMAK_11}
and confidentiality~\citep{Murray_MBGBSLGK_13}, in addition to full \gls{WCET} analysis~\citep{Sewell_KH_16}.
...
To build a dynamic system with temporal isolation and high CPU utilisation, one could provide a high
...
\Cref{tab:nothing-ticks-all-boxes} shows a comparison of all of the available scheduling options in
