...
Therefore our model uses \gls{FP} scheduling as a core part of the kernel.  \gls{EDF}
...
Our model offers two options for enforcement: threads can have an optional timeout fault endpoint,
otherwise the kernel will not permit threads to exceed their budget. 
...
The timeout fault
handler is separate from the existing thread fault endpoint, as the semantics are different. For the
