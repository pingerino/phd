...
4parameters.
\code{sched\_control} is the new control capability, one of which is provided per processing core,
...
\section{Policy and Mechanisms}
\label{sec:model-policy-and-mechanisms}

We now describe several scheduling and resource-sharing policies that can be implemented the
mechanisms of scheduling contexts, scheduling-context donation and timeout faults. 
First, we introduce three new mechanisms which are required to implement those policies, then we 
demonstrate their use to build a range of user-level policies. 

\subsection{Mechanisms}

\subsubsection{Directed yield}
...
directed yield to a specific scheduling context. By invoking the \code{yield\_to} invocation on 
...
a new system call \code{nbsendrecv}. This is required to
facilitate a \emph{forwarding} IPC, and allows passive proxies to seamlessly forward messages,
...
\begin{figure}
    ...
    \begin{subfigure}[h]{0.8\textwidth}
        ...
        \caption{$A$ \code{calls} $S_{1}$ over $E_{1}$.}
        \label{f:forward1}
    \end{subfigure}
    \begin{subfigure}[h]{0.8\textwidth}
        \centering
        \includegraphics[width=\textwidth]{forward2}
        \caption{$S_{1}$ uses \code{nbsendrecv()} to forward the request, and $A$'s resume 
    ...
    \begin{subfigure}[h]{0.8\textwidth}
        ...
        \caption{$S_{2}$ uses the \code{replyrecv()} system call to reply to $A$ and block on
        $E_{2}$ again. }
        \label{f:forward4}
\end{subfigure}
...
\subsubsection{Flexible Resume Capabilities}
...
First, we explain how different real-time task models are compatible with our model and show how
they can be implemented using our mechanisms. 
...
More complicated, time-sharing schedulers can be built by a user level scheduler, using the directed yield and ability to request the amount of time that a specific scheduling context.
