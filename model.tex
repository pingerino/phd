...
\selfour.  However, temporal isolation alone is insufficient: mixed-criticality
...
One might claim that maintaining a scheduler in the kernel violates our goal of policy freedom.
...
problem, which violates temporal isolation.
...
directed yield to a specified scheduling context. By invoking the \scyieldto invocation on 
...
solution forces servers to be multi-threaded, and does not solve the problem of a client's budget
expiring while the server is in a critical section, which is solved by providing atomic primitives
that call the kernel.
