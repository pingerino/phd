...
where code outside of the control of the system designers can be executed.  A modern smart phone is
an open system, given that users can install third-party applications.
...
and can change, as opposed to closed systems which are \emph{fixed} or \emph{static} resource
allocation patterns.
...
timely, guaranteed access to system resources. 
In a resource kernel, a reservation represents a portion of a shared resource, like processor, or
disk bandwidth.  Unlike traditional real-time operation systems, resource kernels do not trust all
applications to stay within their specified resource bounds: resource kernels enforce them,
preventing misbehaving applications from interfering with other applications and thus providing
temporal isolation.
