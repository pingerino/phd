...
allocated and when it is allocated. For most software, time is
...
% Maybe provide an example of non-fungible consequences? Eg: For example, if a result arrives too late it may be useless.
...
correctness of the system. How those deadlines affect correctness depends on the category of the
...
\gls{SRT} tasks can also be found in safety-critical systems where the result degrades if not enough time has been
...
a worst case execution time, $C_{i}$, and period, $T_{i}$, which represents the minimum inter-arrival time
...
% Does schedulable need \emph or is that what feasible is for?
...
\end{table}
% Should the above equation be updated to be <= number of processors instead of 1?
...
describe them briefly here. % Cyclic executives is missing from the above intro I think?
...
\cref{sec:background-edf}
% A ?? gets rendered here in the pdf.
...
preferable. \gls{FP}
% I think this sentence talks about misconceptions that you haven't introduced yet.
...
% Is there a reference for this subsection / I don't get why lock-free and wait-free can't be used?
...
% should the \leq be changed to \geq (or whatever the latex is for greater or equal to)?
...
% This sentence has weird grammar.
