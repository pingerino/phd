...
The infinite series of jobs is per task, and represented by $A_{i1},A_{i2},\ldots,A_{in}$. Each task has
...
this is deciding which thread, or process to run on the processor next. More formally, scheduling
...
is running. All schedulers in known operating systems like Linux, are dynamic, online schedulers. 
...
in a reasonable time for the number of tasks in the task set. Dynamic schedulers conduct a
...
and may leave up to 31\% of the processor idle. \cref{eq:3} shows the schedulability test for
...
additional preemptions, and as a result has poor WCET analysis.

To understand the additional preemptions inherent in \gls{PIP}, consider a task set with $n$ tasks, $A_{1}, \ldots, A_{n}$, where each task's priority
...
atomic compare-and-swap, which is either supported by hardware or provided by disabling preemption. 
