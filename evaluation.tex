...
without activating it such that the kernel will cease switching the FPU context.
\Cref{t:micro} shows the results, and we explore each benchmark in the following sections.
...
\subsection{Fastpath}
To evaluate IPC performance we set up a client (the caller) and server (the callee). We take
timestamps on either side of the IPC operation being benchmarked and record the difference. This is
done 16 times for each result value to warm up the cache, then record the next value. Results
...
This is done for both directions of IPC, as described in \Cref{sec:seL4-api}. 
 The cost of these operations increases by a few percent on each platform,
...
separate new objects (SCO and Resume object) and enforcing priorities
on IPC delivery (baseline does FIFO).
