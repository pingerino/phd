...
% Should you use the full platform+soc names for all of your hardware variants at least once somewhere? Is it reasonable to expect that the tx1 refers to the tegra x1 soc
% being running on a jetson dev board?
\begin{table}[t]
...
% Its weird that you reference the table again as you referenced it at the start of this subsection and the subsection has been referring to it the whole time?
...
microkernel~\citep{Liedtke_95}. We benchmark our \gls{IPC} operations against the base system, \selfour,
...
fastpath. Although we removed the reply capability from the \gls{TCB}
...
\end{table}
% Why were the fastpath microbenchmarks different address spaces, but the slowpath ones same address space.
...
% I don't know what the memory access performance counter means? I would think it means either cycles spent doing memory access, or amount of memory words/bytes read/written, 
% but then that doesn't coincide with the increased instruction count and increased cycle counts.
...
% What is the instruction counter for?  The active table instruction counter shows lower instruction counts than the passive table.
% Does this imply less instructions executed or am i interpreting the counter wrong?
...
and we measure the cost % I think one of your numbers is wrong? 50,000 schedules * 100 runs > 10 000 scheduler invocations?
for 110 runs, abandoning the first 10, so the caches are primed. This results in 10,000 invocations
of the scheduler, as both threads do the switching. Of course, setting the priority of
...
% A sentence at the end of this paragraph linking the whole section back to timing isolation and how your system behaves?
...
\citep{Heiser_Elphinstone_16}.  As our % as on the face of it? 
