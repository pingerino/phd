...
which has an established performant \gls{IPC} fastpath~\citep{Elphinstone_Heiser_13}. 
...
\hline
\end{tabular}
\caption{Microbenchmarks of seL4 fastpath \gls{IPC}.}
...
only overhead on \call is unneccessarily reading the time on kernel entry, as well as the mechanisms
...
mapped and shared between all of the cores. 
...
\hline
\end{tabular}
\caption{Microbenchmarks of seL4 slowpath \gls{IPC} between passive threads.}
\label{t:slowpath-ipc-micro}
\end{table}

\subsubsection{Active slowpath}

Finally we show the cost of slowpath IPC between two active threads. This means that both caller and
callee have a scheduling context, and we bill the thread when we switch threads. In addition to the
...
\hline
\end{tabular}
\caption{Microbenchmarks of seL4 slowpath \gls{IPC} between active threads.}
\label{t:slowpath-ipc-micro}
\end{table}
...
and a faultee. We trigger a fault by executing an undefined instruction in a loop on the faultee's
...
\hline
\end{tabular}
\caption{Microbenchmarks of seL4 fault \gls{IPC} between passive threads.}
