...
invocation, which sends a message to an IPC endpoint (port),
 blocking until it receives until receiving a reply. The server-style operation \code{replyrecv()}
replies to a previous message by invoking the resume object and then blocks, waiting for the next
incoming request. The cost of these operations increases by a few percent on each platform,
