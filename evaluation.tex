...
with the exception of the \textsc{Hikey} which has in order execution compared to all other
...
The fastpath for \replyrecv has a higher impact, for two reasons: first, we add an new capability
lookup, being the resume object provided to receive, which must be looked up by the \replyrecv
fastpath. Although we remove the delete of the previous reply capability from the \gls{TCB}
...
than \call, with the exception of \textsc{Sabre}, which already suffered some \gls{TLB} misses on
...
is to a higher priority thread. The highest overhead is on the \textsc{KZM}, where once again we
...
The \code{signal()} operation on the other hand shows little overhead as it does not alter the
...
compared with the architecture of Redis on Linux, NetBSD and BMK. 
