...
comparable with the in-kernel EDF scheduler from LITMUS$^{RT}$, and
...
The benchmark is by no means a worst-case overhead. For LITMUS$^{RT}$ tasks exercise a single page
of the cache and on seL4 only the instruction cache is effected. This favours seL4 as the scheduler
runs with less cache evictions. However, due to seL4's much smaller cache footprint than
