...
a series of ARM and x86 hardware.
...
isolation in two scenarios, using Redis again, followed by ipbench. Finally, we show isolation in a
shared server, and measure the cost of various timeout fault handling policies. 
...
throughput benchmark to demonstrate minimal, multicore scalability overhead, and we show how server threads can
migrate between processing cores.
...
\begin{tabularx}{\textwidth}{llXlccccc}\toprule
    \emph{Platform}          & \emph{Arch.} & \emph{Microarch.} & \emph{CPU} & \emph{Clock } & L1 & L2 & L3 & TLB  \\
                             &              &                   &            &        GHz    & KiB,$\times$ & KiB, $\times$ & KiB, $\times$ & entries, $\times$ \\\midrule
    \textsc{KZM} (32-bit)    & ARMv6        & ARM1136JF-S       & i.MX31     & 1.0           & 16,4 &
    128,8 & N/A & 32,2\\
    \textsc{Sabre} (32-bit)  & ARMv7        & Cortex-A9         & i.MX6      & 1.0           &
    32+32,4 & 1024, 16 & N/A, 64,2           \\
    \textsc{Hikey} (64-bit)  & ARMv8        & Cortex-A53        & Kirin 620  & 1.2           &
    32+32,4+2 & 512,16 & N/A, 128,4          \\
    \textsc{TX1}   (64-bit)  & ARMv8        & Cortex-A57        & Jetson TX1 & 1.9           &
    32+48,2+3 & 2,048,16 & N/A & 256,4          \\
    \textsc{x64}    (64-bit) & x86          & i7-4770           & Haswell    & 3.1           &
    32+32,8 & 256,8 * 8,192,16, 128,8 \\          
\\\bottomrule\hline
\end{tabularx}
...
cost of the operation\footnote{The \gls{IPC} benchmarks already existed for \selfour, but were modified to
    support the \gls{MCS} kernel during the work for this thesis}. The message sent is zero length, so 
...
CPU allocation provided by their scheduling context. Finally, we evaluate and demonstrate different
...
\begin{enumerate}\label{e:emergency}
    ...
    \item Enqueue the timeout handler itself, with an empty sized request, in the server's
          endpoint queue, by invoking $E_{1}$
    \item Server finishes request with extra budget and replies to the client.
    \item The timeout handler is highest priority, so it overtakes other clients and wakes up after the 
...
\TODO{update - user level criticality not kernel}
...
no deadlines are missed. For stage two, the workload is not
...
% TODO summarise eval
Our evaluation demonstrates our model has minimal overheads, achieves isolation, and allows for efficient user-level scheduling.
